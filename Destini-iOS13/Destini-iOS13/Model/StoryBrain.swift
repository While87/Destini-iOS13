...
struct StoryBrain {
    ...
    var adventureNumber = 0
    
    func getStoryText() -> String {
        return adventure[adventureNumber].title
    }
    
    func getButtonLabel(button: Int) -> String {
        if button == 1 {
            return adventure[adventureNumber].choice1
        }
        else {
            return adventure[adventureNumber].choice2
        }
    }
    
    mutating func nextStory (choice: String) {
        if choice == adventure[adventureNumber].choice1 {
            adventureNumber = adventure[adventureNumber].choice1Destination
        }
        else if choice == adventure[adventureNumber].choice2 {
        adventureNumber = adventure[adventureNumber].choice2Destination
        }
    }
}
